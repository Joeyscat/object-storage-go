...
func TestPutObject(t *testing.T) {
    for _, testCase := range testCases {
        ...
        obj := testCase.Obj
        name := testCase.Name
        hash := hashReaderForUrl(strings.NewReader(obj))
        t.Logf("hash data: %s", hash)

        // put
        resp, err := put("http://jojo:9001/objects/"+name, "", hash, strings.NewReader(obj))
        if err != nil {
...
func hashReaderForUrl(r io.Reader) string {
    h := sha256.New()
    io.Copy(h, r)
